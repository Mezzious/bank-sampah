...
function changeEntries() {
    ...
    var selectedValue = parseInt(select.options[select.selectedIndex].value);
    // Ambil semua baris dalam tabel kecuali baris header
    var rows = document.querySelectorAll("#tableBody");
...
var rowsPerPage = 10; // Jumlah baris per halaman default
var totalRows = 100; // Jumlah total baris

function changeEntries() {
    // Di sini Anda dapat menambahkan logika untuk menangani perubahan jumlah entri yang ditampilkan
    console.log("Changing entries");
    // Misalnya, Anda dapat memuat data baru dari server dengan jumlah entri yang baru
}

function updatePageInfo() {
    var infoElement = document.getElementById("example_info");
    var startRow = (currentPage - 1) * rowsPerPage + 1;
    var endRow = Math.min(currentPage * rowsPerPage, totalRows);
    infoElement.innerText = "Showing " + startRow + " to " + endRow + " of " + totalRows + " entries";
}

function updateTable() {
    var rows = document.querySelectorAll("#tableBody tr");
    var startIdx = (currentPage - 1) * rowsPerPage;
    var endIdx = Math.min(startIdx + rowsPerPage, rows.length);
